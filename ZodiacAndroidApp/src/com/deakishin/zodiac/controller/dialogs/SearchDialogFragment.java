...
public class SearchDialogFragment extends DialogFragment {
	...
	/* Количество поисковых слов, при котором выводится предупреждение. */
	private static final int WORDS_COUNT_TO_WARN = 2;
		...
		mSearchButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				if (mTaskIsRunning) {
					...
					if (words.length >= WORDS_COUNT_TO_WARN) {
						if (!mSettingsPersistent.isSearchWarningDontShow()) {
							showWarning();
							return;
						}
					}
	...
	private void enableWidgets(boolean toEnable) {
		...
		mHelpButton.setEnabled(toEnable);
	...
	/* Исправление бага с уничтожением фрагмента при повороте. */
	@Override
